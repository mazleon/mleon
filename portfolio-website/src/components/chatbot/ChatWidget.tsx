...
export default function ChatWidget() {
  ...
  const { messages, remainingMessages, isExpired, addMessage } =
    useChatSession();
  ...
  return (
    <>
      ...
      <motion.button
        ...
        className={`fixed bottom-6 right-6 z-50 w-14 h-14 rounded-full bg-accent text-white shadow-lg shadow-accent/30 flex items-center justify-center hover:scale-110 transition-transform ${
          isOpen ? "hidden" : ""
        }`}
          ...
          >
              ...
              {messages.length === 0 && (
                <div className="text-center py-8">
                  ...
                  <h4 className="text-white font-medium mb-2">
                    Hi! I'm {PORTFOLIO_CONTEXT.personal.name.split(" ")[0]}'s AI
                    assistant
                  </h4>
            ...
            {isExpired && (
              <div className="px-4 py-3 bg-red-500/10 border-t border-red-500/20 flex items-center gap-2 text-red-400 text-sm">
                <AlertCircle size={16} />
                <span>
                  Session limit reached. Refresh the page for a new session.
                </span>
              </div>
            )}
