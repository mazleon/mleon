...
const skillCategories: SkillCategory[] = [
  ...
  },
  {
    ...
    skills: [
      { name: "Spatiotemporal Deep Learning" },
      { name: "Object Detection (YOLO family)" },
      { name: "Face Recognition Systems" },
      { name: "OCR (TrOCR)" },
      { name: "Biometric Pipeline Design" },
      { name: "Deep Learning for Health/Environment Data" },
    ],
  },
  {
    id: "tier3",
    ...
    skills: [
      ...
      { name: "Vision-Language Models (LLaVA / Florence-2)" },
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {skillCategories.map((category, index) => (
            <MotionWrapper
              ...
              delay={index * 0.05}
              className={
                category.span === "wide" ? "md:col-span-2" : "col-span-1"
              }
            >
              <div className="editorial-card h-full group hover:border-accent/20 cursor-default">
                ...
                <div className="flex flex-wrap gap-2">
                  {category.skills.map((skill) => (
                    ...
                    >
                      {skill.icon && (
                        <span className="text-muted">{skill.icon}</span>
                      )}
                      {skill.name}
                    </span>
                  ))}
          ...
          <div className="flex flex-wrap justify-center items-center gap-x-8 gap-y-3">
            {[
              ...
              <div key={comp} className="flex items-center gap-3">
                ...
                <span className="text-sm font-mono tracking-wider text-muted uppercase">
                  {comp}
                </span>
              </div>
            ))}
